...
class BasicStrategyAgent(Agent):
    ...
    def _initialize_hard_strategy(self):
        ...
        for player_sum in range(3, 9):
            self.hard_total_action[player_sum] = {dealer_card: Action.HIT for dealer_card in range(2, 12)}
        self.hard_total_action[9] = {dealer_card: Action.DOUBLE for dealer_card in range(3, 7)}
        self.hard_total_action[9].update({dealer_card: Action.HIT for dealer_card in [2] + list(range(7, 12))})
        self.hard_total_action[10] = {dealer_card: Action.DOUBLE for dealer_card in range(2, 12)}
        self.hard_total_action[10].update({dealer_card: Action.HIT for dealer_card in range(10, 12)})
        self.hard_total_action[11] = {dealer_card: Action.DOUBLE for dealer_card in range(2, 12)}
        ...
        for player_sum in range(17, 22):
            self.hard_total_action[player_sum] = {dealer_card: Action.STAY for dealer_card in range(2, 12)}
            
    def _initialize_soft_strategy(self):
        """Creates the basic strategy used by the agent when there is a usable ace."""
        for player_hand in range(3, 13):
            self.soft_total_action[player_hand] = {dealer_card: Action.HIT for dealer_card in range(2, 12)}
        ...
        for player_hand in range(15, 18):
            ...
            self.soft_total_action[player_hand][5] = Action.DOUBLE
            self.soft_total_action[player_hand][6] = Action.DOUBLE
        self.soft_total_action[18] = {dealer_card: Action.STAY for dealer_card in range(2, 12)}
        ...
        for player_hand in range(19, 22):
            self.soft_total_action[player_hand] = {dealer_card: Action.STAY for dealer_card in range(2, 12)}
